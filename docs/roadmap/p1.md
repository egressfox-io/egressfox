...
Status: proposed implementation plan. No P1 product behavior is implemented.
...
designs define detailed semantics. Future implementation plans may decompose one
milestone into focused branches, but must not silently change this sequence or add
scope.
...
3. M9 — target-aware selection profiles over one shared inventory;
4. M10 — one bounded `EgressPolicy` per gateway and common ordered routing;
5. M11 — a low-cardinality operational metrics contract;
6. M12 — bounded explainability CLI and P1 acceptance scenarios.
...
- one additional protocol slice chosen from usage evidence and renderer parity;
...
- Vault, generic webhook/HTTP output and arbitrary ConfigMap output;
...
- Web UI, rich TUI, Rust workspace, predictive selection and custom scoring code;
- broad protocol or engine-version expansion.
...
to precede managed HTTP refresh. Vault and generic outputs each introduce an
independent authentication, acknowledgment and recovery contract and are not added
for adapter symmetry.
...
flowchart LR
    ...
    M8 --> M9[M9 target-aware profiles]
    M9 --> M10[M10 EgressPolicy routing]
    M10 --> M11[M11 operational metrics]
    M11 --> M12[M12 explainability and P1 acceptance]
```

M8 and M9 have little code dependency on one another, but the sequence is deliberate:
M8 removes the need for users to keep subscription Secrets refreshed before P1
multiplies target profiles over that inventory. M10 needs named profile outputs.
...
and transactional snapshot boundary. A 304 reuses a matching cache; it is never an
empty source. Cache fallback is time-bounded and never converts an acquisition or
parse failure into authoritative empty input.
...
DNS behavior, references, infeasible groups, status and migration.
...
a bounded ordered rule list, named references to pool profiles and an explicit final
action. The initial target is exact/suffix domain, CIDR and TCP destination-port
...
ordered matches/actions and explicit final behavior. Keep selection output separate
from group composition. Validate missing/empty groups, cycles, rule order, duplicate
names and unsupported semantics before rendering. Renderers reject rather than
weaken engine asymmetry.
...
**Exit criteria.** One Gateway renders and activates a deterministic policy using at
least two named profile selections plus explicit final behavior on both engines;
unsupported or infeasible policy retains LKG; BYO and legacy implicit policy still
work.
...
compatibility surface before release.
...
and activation layers supportable. A narrow CLI is justified by this concrete use
case; a second daemon or UI is not.

**Dependencies and entry gates.** M7–M11; Q11 and Q17 must fix command/exit behavior,
authorization, bounded report schema, opt-in disclosure and retention. No CLI reads
raw Secrets or SQLite over an ad hoc path.
...
remain visible as a count rather than disappearing silently.
...
**Non-goals.** Full standalone daemon, `fetch`/`probe`/`render` command suite, raw
configuration diff, kubectl plugin packaging, Web UI, TUI or Rust workspace.
