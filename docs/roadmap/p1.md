...
3. M8.5 — protocol and transport compatibility expansion after focused M8
   hardening; Reality/Vision and any further variant need typed semantics,
   native validation and controlled traffic through both supported engines;
...
This next bounded stage is unimplemented. The M8 hardening pass records stable
subscription identity, strict source-destination permissions and Linux managed
traffic proof for the current VMess/Shadowsocks subset. M8.5 will need its own
design gates and execution plan for Reality/Vision and any further protocol or
transport variants. A parser path alone cannot establish support: both pinned
renderers, native validation, probes and controlled traffic must preserve complete
connection semantics. It does not change EgressFox routing ownership or introduce
