...
| Q23 | C1 forbids relaxing existing destination authorization. C4 must define precise Hysteria2/QUIC port-hopping, realm/STUN and other secondary destination permissions, UDP DNS/rebinding checks, and through-engine probe attribution. | M8.5 C4 before UDP implementation | [ADR 0021](0021-version-three-connection-semantics-and-capabilities.md), [engine matrix](../designs/engine-protocol-compatibility.md) and [testing](../development/testing.md) |
| Q24 (C4 qualification gate) | [ADR 0023](0023-c4-hysteria2-quic-network-boundary.md) selects sing-box build revision 3 with `with_quic,with_utls` and a bounded Hysteria2 subset. The maintainer's exact native and traffic qualification remains required before a support claim. | Before sing-box C4 Hysteria2 support claims | [Engine matrix](../designs/engine-protocol-compatibility.md) and [release manifest](../../release/manifest.json) |
...
| Q21 | [ADR 0021](0021-version-three-connection-semantics-and-capabilities.md) defines v3 typed connection identity and confidential revision fields while preserving v1/v2 bytes and SQLite evidence. |
| Q22 | The [exact-profile matrix](../designs/engine-protocol-compatibility.md) records pinned source/build support, asymmetries and unverified parameter families; ADR 0021 defines the EgressFox capability gate. Each C2–C4 claim still needs native and traffic proof. |
