...
| HTTP(S)/SOCKS proxy supplied by an untrusted subscription observes traffic or receives credentials | Treat proxy credentials as confidential revisions, authorize each proxy server separately from probe targets, require an explicit proxy-entry input context, preserve TLS-to-proxy verification and reject unsupported auth/headers rather than dropping them; enforce network-level egress policy as defense in depth |
